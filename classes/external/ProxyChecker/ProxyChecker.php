...
class ProxyChecker
{
    ...
    private function getProxyContent($proxy)
    {
        ...
        $options = array(
            ...
            CURLOPT_HEADER         => true,
            CURLOPT_TIMEOUT        => $this->config['timeout'] + 11,
            CURLOPT_CONNECTTIMEOUT => $this->config['timeout'] - 3,
    ...
    private function checkProxyContent($content, $info)
    {
        ...
        // proxy level
        $proxyLevel = '';
        if (mb_strpos($content, 'proxylevel_elite')) {
            $proxyLevel = 'elite';
        } elseif (mb_strpos($content, 'proxylevel_anonymous')) {
            $proxyLevel = 'anonymous';
        } elseif (mb_strpos($content, 'proxylevel_transparent')) {
