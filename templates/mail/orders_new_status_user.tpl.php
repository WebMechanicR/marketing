...
<body style="font:12px Arial, Tahoma, Verdana, sans-serif">
							<h1>Здравствуйте, <?php echo $order['name'];?>.</h1>
                            
                            <p>Статус Вашего заказа №<?php echo $order['id'];?> от <?php echo date('d.m.Y', $order['date_add']);?> изменился на <strong><?php echo $order_statuses[$order['status']]['name'];?></strong>.</p>
                            <?php if(isset($order['secret_invoice']) and $order['secret_invoice'] and $order['file_invoice']) { ?>
                            <p>К заказу прикреплен счет для оплаты - <a href="<?php echo SITE_URL;?>invoice_<?php echo $order['secret_invoice'];?>.htm">скачать</a>. Счет действителен 2 недели.</p>
           					<?php } ?>                 
                            ...
                            <?php 
								if(isset($order) and isset($order['phones'])) { 
									$order['phones'] = unserialize($order['phones']);
									...
									<tfoot>
										<tr>
											<td colspan="5" align="right" style="padding-top:15px; padding-bottom:15px; border-top: 1px solid #990000;  border-bottom: 1px solid #990000">
												<p><font style="font-size:16px;">Ваш заказ: <?php echo $order['amount'].' '.F::get_right_okonch($order['amount'], "товаров", "товар", "товара");?> на сумму <?php echo F::number_format($order['total_price']);?> руб.</font></p>
											</td>
										</tr>
									</tfoot>
								</table><br><br>
