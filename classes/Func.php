...
class Func {
	...
	static function unclean($string) {
        ...
        static function phoneBlocks($phone){
		if (empty($phone)) return "";
		if (strlen($phone) == 7)
			sscanf($phone, "%3s%2s%2s", $prefix, $exchange, $exchange2);
		else if (strlen($phone) == 10)
			sscanf($phone, "%3s%3s%2s%2s", $area, $prefix, $exchange, $exchange2);
		else if (strlen($phone) > 10) {
			if(substr($phone, 0,1)=='1') {
				sscanf($phone, "%1s%3s%3s%4s", $country, $area, $prefix, $exchange);
			}
			else{
				sscanf($phone, "%3s%3s%2s%2s%s", $area, $prefix, $exchange, $exchange2, $extension);
			}
		}
		else
			return '';
	 ...
	 * @return string
	 */
	static function generate_passw($len=6) {
		$c = "";
		for ($i=0;$i<$len;$i++) {
			$rand=rand(1,3);
			if($rand==1) $c.=chr(rand(0x30, 0x39));
			elseif($rand==2) $c.=chr(rand(0x41, 0x5a));
			elseif($rand==3) $c.=chr(rand(0x61, 0x7a));
		}
		return $c;
	}
	...
	static function get_right_okonch($numeric, $many, $one, $two) {
        ...
        static function format_str($str, $length){
	...
	static function mutex_lock($mut_name = "", $unlock = false){
	    if(!defined("ROOT_DIR_SERVICE"))
		define("ROOT_DIR_SERVICE", __DIR__.'/');
	    
	    static $mutexes = array();
	    if(!$mut_name)
		$mut_name = 'st';
	    
	    if(!$unlock){
		if(!is_dir(ROOT_DIR_SERVICE.'mutexes/'))
			mkdir(ROOT_DIR_SERVICE.'mutexes/');
		
		$mutexes[$mut_name] = fopen(ROOT_DIR_SERVICE.'mutexes/mutex_'.$mut_name.'.mut', 'a+');
		
		if(!$mutexes[$mut_name])
		    return false;
		
		if(!flock($mutexes[$mut_name], LOCK_EX | LOCK_NB)){
		    fclose($mutexes[$mut_name]);
		    return false;
		}
		
		flock($mutexes[$mut_name], LOCK_EX);
		return true;
	    }
	    else{
		 if(isset($mutexes[$mut_name]) and $mutexes[$mut_name]){
		     @flock($mutexes[$mut_name], LOCK_UN);
		     @fclose($mutexes[$mut_name]);
		     unset($mutexes[$mut_name]);
		 }
		 return true;
	    }
	}
	
	static  function flush_to_non_blocking_socket() {
	    @ob_end_clean();
	    ignore_user_abort();
	    ob_start();
